...
export const layers = [
  ...
  },
  {
    ...
    readMoreUrl: 'https://endpoints.huggingface.co/',
...
export const tools = [
  ...
  makeTool('tiktoken', 'tiktoken', 'client library', 'input', ['gpu', 'cpu'], 'Fast BPE tokenization used to estimate and prepare model inputs.', 'You need predictable token counting for OpenAI-compatible model workflows.', ['litellm', 'gptcache'], 'https://github.com/openai/tiktoken'),
  ...
  makeTool('llmlingua', 'LLMLingua', 'prompt optimizer', 'input', ['gpu', 'cpu'], 'Prompt-compression toolkit intended to reduce input-token cost and latency.', 'Long prompts are repeated or cost-sensitive and you can validate compression quality.', ['gptcache', 'redis-langcache'], 'https://github.com/microsoft/LLMLingua'),
  ...
  makeTool('openrouter', 'OpenRouter', 'model router', 'gateway', ['gpu', 'cpu'], 'Unified API and marketplace router across hosted model providers.', 'You want application-level access to many hosted models through one endpoint.', ['litellm', 'aws-bedrock'], 'https://openrouter.ai/'),
  makeTool('cloudflare-ai-gateway', 'Cloudflare AI Gateway', 'gateway', 'gateway', ['gpu', 'cpu'], 'Edge gateway for provider routing, caching, rate limiting, and observability.', 'Your application already benefits from Cloudflare’s edge network and controls.', ['redis-langcache', 'aws-bedrock'], 'https://developers.cloudflare.com/ai-gateway/'),
  ...
  makeTool('langchain-cache', 'LangChain cache', 'cache interface', 'response-cache', ['gpu', 'cpu'], 'Cache interfaces and integrations for application-level LLM response reuse.', 'Your application already uses LangChain and needs interchangeable cache backends.', ['gptcache', 'redis-langcache'], 'https://docs.langchain.com/oss/javascript/integrations/llm_caching'),
  ...
  makeTool('radixattention', 'RadixAttention', 'cache primitive', 'kv-cache', ['gpu'], 'Radix-tree prefix cache used by SGLang to maximize reuse of shared prompts.', 'Requests share long system prompts, documents, or conversation prefixes.', ['sglang', 'hicache'], 'https://docs.sglang.io/'),
  makeTool('hicache', 'HiCache', 'cache tier', 'kv-cache', ['gpu'], 'Hierarchical cache mechanism for extending SGLang KV reuse beyond GPU memory.', 'Large-context workloads need a staged GPU, host-memory, or storage cache hierarchy.', ['sglang', 'mooncake'], 'https://docs.sglang.io/docs/advanced_features/hicache'),
  ...
  makeTool('lmcache', 'LMCache', 'distributed cache', 'kv-cache', ['gpu'], 'PyTorch Foundation member project (October 2025) with 9.2K+ GitHub stars. Makes KV cache reusable and persistent across serving engines, integrated with NVIDIA Dynamo, the vLLM production-stack, IBM, and CoreWeave, delivering 3-10x faster inference through cross-request KV reuse.', 'Long-context prompts recur across a fleet of vLLM or SGLang workers.', ['vllm', 'sglang', 'mooncake'], 'https://github.com/LMCache/LMCache'),
  makeTool('mooncake', 'Mooncake', 'distributed cache', 'kv-cache', ['gpu'], 'Winner of the FAST 2025 Best Paper Award. A Moonshot AI project and LMCache partner providing a high-performance, disaggregated prefill-focused distributed KV-cache store and transfer engine for KV-centric inference.', 'You need remote KV reuse and high-throughput transport across an inference cluster.', ['lmcache', 'nixl'], 'https://github.com/kvcache-ai/Mooncake'),
  makeTool('nixl', 'NIXL', 'data transfer', 'kv-cache', ['gpu'], "NVIDIA's Network Interconnect Library for fast KV cache transfer between disaggregated prefill and decode workers. An official NVIDIA project used as the transport backend in LMCache and NVIDIA Dynamo.", 'Your disaggregated NVIDIA serving path needs a standard high-performance transfer layer.', ['nvidia-dynamo', 'trt-kv-cache'], 'https://github.com/ai-dynamo/nixl'),
  ...
  makeTool('tgi', 'Hugging Face TGI', 'serving engine', 'serving', ['gpu'], 'Hugging Face text-generation server with batching and production operations features. The repository was archived (read-only) on 21 Mar 2026; Hugging Face now recommends vLLM or SGLang for new deployments.', 'You are maintaining an established, already-deployed Hugging Face TGI installation and need compatibility context — not for new deployments.', ['hf-tokenizers', 'hf-optimum'], 'https://github.com/huggingface/text-generation-inference', 'archived'),
  makeTool('deepspeed-fastgen', 'DeepSpeed-FastGen', 'serving engine', 'serving', ['gpu'], 'DeepSpeed inference stack for fast, low-latency transformer generation.', 'Your deployment already relies on DeepSpeed optimizations or compatible model flows.', ['triton', 'awq'], 'https://github.com/deepspeedai/DeepSpeed'),
  makeTool('lmdeploy', 'LMDeploy', 'serving engine', 'serving', ['gpu'], 'Inference and serving toolkit for efficient deployment of foundation models.', 'You want a serving path with strong support for model compression and deployment tools.', ['awq'], 'https://github.com/InternLM/lmdeploy'),
  makeTool('aphrodite', 'Aphrodite Engine', 'serving engine', 'serving', ['gpu'], 'OpenAI-compatible inference engine derived from the vLLM ecosystem with broad model support. Originally maintained by PygmalionAI, the project has since moved to the dphnAI org and been renamed Sonar.', 'You need an alternative vLLM-compatible serving engine for supported model families.', ['vllm', 'pagedattention'], 'https://github.com/dphnAI/sonar'),
  makeTool('mistral-rs', 'mistral.rs', 'serving engine', 'serving', ['gpu', 'cpu'], 'Fast, flexible Rust-based LLM inference engine built by Eric Buehler (~14K+ GitHub stars). Supports GGUF and Hugging Face models, 2-8 bit quantization, Flash Attention v2, LoRA and X-LoRA MoE, with Metal, CUDA, and MKL backends. A community favorite for performance and the Rust inference ecosystem.', 'You want a fast, memory-safe Rust serving engine with broad quantization and LoRA support across Metal, CUDA, or CPU backends.', ['gguf', 'llama-cpp'], 'https://github.com/EricLBuehler/mistral.rs'),
  ...
  makeTool('mlx', 'MLX', 'local runtime', 'runtime', ['cpu', 'gpu'], 'Apple Silicon array framework and LLM ecosystem for local Mac inference, executing on the Apple GPU via Metal over unified memory rather than the CPU alone.', 'You are optimizing local inference for Apple Silicon developers or endpoints.', ['llama-cpp', 'ollama'], 'https://github.com/ml-explore/mlx'),
  makeTool('webllm', 'WebLLM', 'browser runtime', 'runtime', ['cpu'], 'WebGPU-based in-browser LLM inference engine.', 'Privacy, offline availability, or zero-install browser execution is more important than fleet throughput.', ['xgrammar'], 'https://github.com/mlc-ai/web-llm'),
  ...
  makeTool('trt-model-optimizer', 'TensorRT Model Optimizer', 'compiler', 'runtime', ['gpu'], 'NVIDIA toolkit for quantizing and optimizing models for TensorRT-LLM deployment.', 'You are preparing NVIDIA-targeted models for TensorRT-LLM performance.', ['tensorrt-llm', 'trt-kv-cache'], 'https://github.com/NVIDIA/Model-Optimizer'),
  ...
  makeTool('llmd', 'llm-d', 'distributed runtime', 'distributed', ['gpu'], 'CNCF Sandbox project (accepted March 2026) founded by Red Hat, Google, IBM, NVIDIA, and CoreWeave. Kubernetes-native distributed LLM inference stack built on vLLM with AI-aware network routing, KV-cache-aware scheduling, and prefill/decode disaggregation. v0.5 adds hierarchical KV offloading, cache-aware LoRA routing, and scale-to-zero, and has been validated at 50k tok/s on 16xB200.', 'You need an open control plane for a large vLLM fleet on Kubernetes.', ['llmd-kv-cache', 'kserve'], 'https://github.com/llm-d/llm-d'),
  makeTool('kserve', 'KServe', 'operator', 'distributed', ['gpu', 'cpu'], 'Kubernetes model-serving platform with inference graph and autoscaling capabilities.', 'You need a standardized Kubernetes serving control plane across model types.', ['gateway-inference-extension', 'kuberay'], 'https://github.com/kserve/kserve'),
  ...
  makeTool('hf-inference-endpoints', 'Hugging Face Inference Endpoints', 'managed endpoint', 'managed', ['gpu', 'cpu'], 'Managed dedicated endpoints for deployed Hugging Face models.', 'You want managed deployment of an open model with Hugging Face tooling.', ['hf-tokenizers', 'tgi'], 'https://endpoints.huggingface.co/'),
  ...
  makeTool('google-vertex-ai', 'Google Gemini Enterprise Agent Platform', 'managed endpoint', 'managed', ['gpu', 'cpu'], 'Managed AI development and agent-building platform on Google Cloud; Google reorganized Vertex AI into this unified platform in April–May 2026.', 'Your data, governance, and model workflows are centered on Google Cloud.', ['litellm', 'openrouter'], 'https://cloud.google.com/products/gemini-enterprise-agent-platform'),
  makeTool('azure-ai-foundry', 'Microsoft Foundry', 'managed endpoint', 'managed', ['gpu', 'cpu'], 'Microsoft platform for deploying and governing models and AI applications; renamed from Azure AI Foundry to Microsoft Foundry effective 1 Jan 2026.', 'Your enterprise model access and controls are centered on Microsoft Azure.', ['litellm', 'onnxruntime-genai'], 'https://azure.microsoft.com/products/ai-foundry'),
